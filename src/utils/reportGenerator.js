...
export const generatePDFReport = async (reportData, fileName = 'report.pdf') => {
  try {
    ...
    // Additional Insights (for a more complete report feel)
    const insights = [
      {
        title: 'Highlights',
        body: 'Key wins and completed milestones during this period. Progress trends are positive with steady completion rate.',
      },
      {
        title: 'Risks & Blockers',
        body: 'Potential risks include dependency delays and resource constraints. No critical blockers detected at generation time.',
      },
      {
        title: 'Recommendations',
        body: 'Prioritize high-impact tasks, rebalance workload, and review sprint scope for the next planning cycle.',
      },
    ];
    ...
    insights.forEach((item) => {
      ...
      yPosition += 4;
      doc.setFont(undefined, 'normal');
      const lines = doc.splitTextToSize(item.body, maxWidth);
      lines.forEach((line) => {
    ...
    doc.setTextColor(...lightGray);
    doc.text(
      'Report generated by Track-Flow Managed By Vedant Trivedi',
      pageWidth / 2,
      pageHeight - 16,
      { align: 'center' }
    );
...
export const generateCSVReport = (reportData, fileName = 'report.csv') => {
  try {
    ...
    const header = [
      ...
      [],
      ['Key Metrics'],
      ...(reportData.metrics || []).map((m) => [m.label, String(m.value)]),
      [],
      ['Insights'],
      ['Highlights', 'Key wins and completed milestones during this period.'],
      ['Risks & Blockers', 'Potential risks include dependency delays and resource constraints.'],
      ['Recommendations', 'Prioritize high-impact tasks and rebalance workload.'],
      [],
    ];
